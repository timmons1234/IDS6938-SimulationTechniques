...
void SIMAgent::InitValues()
{
	...
	*********************************************/
	Kv0			= 10.0;		// Agent Velocity
	Kp1			= -200.0;	// Agent Heading
	Kv1			= 5.0;		// Agent Heading also
	KArrival	= 10.0;		// 
	KDeparture	= 20.0;		//
	KNoise		= 1.0;		//
	KWander		= 1.0;		//
...
vec2 SIMAgent::Seek()
{
	...
	*********************************************/
	...
	tmp = goal - GPos;
	tmp.Normalize();
	// From first class where we wrote these functions out
	// arctan(v.y / v.x) == atan2 (tmp[1]/tmp[0])
...
vec2 SIMAgent::Arrival()
{
	...
	*********************************************/
	...
	float Vd = tmp.Length()*KArrival;
	float Vn = (SIMAgent::MaxVelocity*(vd / radius));
	//float Vn = (SIMAgent::MaxVelocity * tmp.Length() / radius);
...
vec2 SIMAgent::Departure()
{
	...
	*********************************************/
	...
	float Vd = tmp.Length()*KDeparture;
	float Vn = (SIMAgent::MaxVelocity*(vd / radius));
...
vec2 SIMAgent::Avoid()
{
	...
	*********************************************/
	...
	toobj1 = GPos + v0.Normalize() * KAvoid;
	toobj2 = GPos + v0.Normalize() * KAvoid *.5;

	for (int i = 0; i < env->obstaclesNum; i++)
	{
		obstacle[0] = env->obstacles[i][0];
		obstacle[1] = env->obstacles[i][i]; 
		float dist1 = (obstacle + toobj1).Length();
		float dist2 = (obstacle + toobj2).Length();

		if (dist1 <= env->obstacles[i][2] + KAvoid ||
			dist2 <= env->obstacles[i][2] + KAvoid)
		{
			thetad = thetad + TAvoid;
			ClampAngle(thetad);
			vd = SIMAgent::MaxVelocity * (dist1 / radius);
			return vec2(cos(thetad)*vd, sin(thetad)*vd);
		}
